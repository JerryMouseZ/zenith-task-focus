...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `你是一个智能任务解析助手。请将以下文本解析为一个 JSON 对象，包含以下字段：
    - title: 任务的核心内容
    - description: 额外细节（可选）
    ...
    - priority: 从 'urgent', 'high', 'medium', 'low' 中选择，默认为 'medium'
    - tags: 字符串数组，从文本中提取人名、项目名等
    - contextTags: 字符串数组，从 '@电脑前', '@通勤路上', '@碎片时间', '@会议室', '@家中', '@外出办事', '@休息时间' 中选择合适的上下文标签
    - energyLevel: 从 'high', 'medium', 'low' 中选择，根据任务复杂度和专注度要求判断
    ...
    优先级判断：
    - 'urgent': 包含"紧急"、"立即"、"马上"、"急需"等词语
    - 'high': 包含"重要"、"优先"、"关键"、"尽快"等词语
    - 'medium': 包含"一般"、"常规"、"正常"等词语或默认情况
    - 'low': 包含"不急"、"有空时"、"可以延后"等词语
    
    能量级别判断：
    - 'high': 需要深度思考、创意工作、学习、写作、编程、设计等复杂任务
    - 'medium': 会议、沟通、处理邮件、日常工作、简单分析等
    - 'low': 整理文件、查看资料、简单回复、日常维护等轻松任务
    
    上下文标签判断：
    - '@电脑前': 需要电脑操作，编程、写作、设计、办公等
    - '@通勤路上': 可在移动中完成，阅读、听音频、简单思考等
    - '@碎片时间': 短时间内可完成，查看消息、简单回复等
    - '@会议室': 需要会议环境，团队讨论、汇报、培训等
    - '@家中': 个人事务、休息相关、家庭事务等
    - '@外出办事': 需要外出，购买、办证、见面等
    - '@休息时间': 轻松活动、娱乐、运动等
