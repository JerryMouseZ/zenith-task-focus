    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div>
            ...
            {isEditing ? (
              ...
              <div className="flex items-center gap-2">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-900">{editedTask.title}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <Badge className={`${priorityColors[editedTask.priority as TaskPriority] || ""} text-xs sm:text-sm px-2 py-1`}>
          ...
          {isEditing && (
            <div className="flex items-center space-x-2 pt-2">
              <Checkbox
                ...
                className="w-3.5 h-3.5 sm:w-4 sm:h-4"
              />
              <label htmlFor="fixedTime" className="text-xs sm:text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                固定时间任务（不能被自动调整时间）
              </label>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              ...
              {isEditing ? (
                ...
                <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-gray-600">
                  ...
                  {editedTask.dueDate ? editedTask.dueDate.toLocaleString([], { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit'}) : "无截止时间"}
                </div>
              )}
            </div>
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-gray-600">
                  <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  {editedTask.startTime ? editedTask.startTime.toLocaleString([], { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit'}) : "无开始时间"}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              <label htmlFor="recurrence" className="text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2 block">
                循环设置
              </label>
              <Select
                ...
                disabled={!isEditing}
              >
                <SelectTrigger id="recurrence" className="text-xs sm:text-sm">
                  <SelectValue placeholder="选择循环频率" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="none" className="text-xs sm:text-sm">不循环</SelectItem>
                  <SelectItem value="daily" className="text-xs sm:text-sm">每天</SelectItem>
                  <SelectItem value="weekly" className="text-xs sm:text-sm">每周</SelectItem>
                  <SelectItem value="monthly" className="text-xs sm:text-sm">每月</SelectItem>
                </SelectContent>
              </Select>
            </div>
            {editedTask.recurrence && editedTask.recurrence !== 'none' && (
              <div>
                <label htmlFor="recurrence_end_date" className="text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2 block">
                  循环结束日期
                </label>
                <Input
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2">
              ...
              {(!editedTask.tags || editedTask.tags.length === 0) && !isEditing && (
                <span className="text-xs sm:text-sm text-gray-500">暂无标签</span>
              )}
            </div>
            {isEditing && (
              <div className="flex gap-2">
                <Input
                  ...
                  className="text-xs sm:text-sm"
                />
                <Button onClick={handleAddTag} size="sm" className="text-xs sm:text-sm px-3">
                  添加
                </Button>
              </div>
            )}
          ...
          {task && (
            <div>
              ...
              <div className="space-y-1.5 sm:space-y-2 mb-2 max-h-32 sm:max-h-40 overflow-y-auto">
                ...
                {childTasks.length === 0 && (
                  <div className="text-xs sm:text-sm text-gray-500 p-2 text-center">暂无子任务</div>
                )}
              </div>
              <div className="flex">
                <Button onClick={() => setShowSubtaskModal(true)} size="sm" className="text-xs sm:text-sm px-3">
                  <Plus className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1" /> 添加子任务
                </Button>
              </div>
              {/* 子任务弹窗 */}
              {showSubtaskModal && (
